...
const locate_rule = function(user_query, fields)
{
  ...
  {
    let parts = user_query.split(operators[o])
...
query_parser.parse = function(user_query, fields)
{
  ...
  user_query = user_query.replaceAll(' and ', '&&').replaceAll(')and ', ')&&').replaceAll(' and(', '&&(').replaceAll(')and(', ')&&(')
  user_query = user_query.replaceAll(' or ', '||').replaceAll(')or ', ')||').replaceAll(' or(', '||(').replaceAll(')or(', ')||(')
