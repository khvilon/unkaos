  ...
  export default 
  {
    ...
    methods:
    {
      get_node_by_uuid(uuid)
      {
        ...
        {
          if(this.wdata.workflow_nodes[i].issue_statuses[0].uuid === uuid) return this.wdata.workflow_nodes[i]
        }
      },
      get_edge_by_uuids(uuids)
      {
        ...
        for(let i in this.wdata.transitions)
        {
          ...
          if(tr.status_from_uuid === from_uuid && tr.status_to_uuid == to_uuid) return tr
        }
      },
      calc_edge_d(transition, is_bidirectional)
      {
        ...
        return graph_math.calc_edge_d(node_from, node_to, this.node_radius, is_bidirectional)
      },
      are_bidirectional(edge0, edge1)
      {
        return edge0.status_from_uuid == edge1.status_to_uuid &&
      ...
      calc_are_bidirectional_edges()
      {
        ...
        for(let i in this.wdata.transitions)
        {
          ...
          {
              let are_bidirectional = this.are_bidirectional(this.wdata.transitions[i], this.wdata.transitions[j])
              if(are_bidirectional) are_bidirectional_array[i] = are_bidirectional_array[j] = true
          }
        }
      ...
      update_edges(node_uuid) 
      {
        ...
        {
          let tr = this.wdata.transitions[i]
          if(tr.status_from_uuid != node_uuid && tr.status_to_uuid != node_uuid && node_uuid != undefined) continue
      ...
      node_mouseup($event)
      {
        ...
        if(this.dragged_node.issue_statuses[0].uuid == node.issue_statuses[0].uuid) return
        
        this.create_edge(this.dragged_node.issue_statuses[0].uuid, node.issue_statuses[0].uuid) 
      },
      svg_mousedown($event)
      {
      ...
      mouse_move($event)
      {
        ...
        let node = this.get_node_by_uuid(this.dragged_node.issue_statuses[0].uuid)
...
//console.log('nnn', this.$store['selected_workflows'])
      ...
      format_node_text(text)
      {
        ...
        for(let i in words)
        {
          let y = -(line_height*(words.length-1))/2 + line_height*(i) + line_up
      ...
      create_node()
      {
        ...
        if(this.get_node_by_uuid(st.uuid) != undefined) {console.log('status already exists'); return}
      ...
      delete_element()
      {
        if(this.selected.node != undefined)
        {
          ...
          this.wdata.workflow_nodes.splice(this.wdata.workflow_nodes.indexOf(this.selected.node), 1);
        }
   ...
   //   console.log('iss1')
    ...
    computed:
    {
      issue_statuses: function(){  return this.$store.getters.get_issue_statuses },
  	  selected_issue_statuses: function(){ return this.$store.getters.selected_issue_statuses }
    }
  }
</script>
        ...
        >
          ...
          <g class="graph">
            ...
            <g>
              ...
              v-bind:class="{ selected: selected.edge != undefined && selected.edge.uuid == edge.uuid}"
              ></path>
            </g>
            <g>
              <g 
              ...
              v-bind:class="{ selected: selected.node != undefined && selected.node.issue_statuses[0].uuid == node.issue_statuses[0].uuid}"
              >
...
<style >
  ...
  .workflows-command-panel .string, .workflows-command-panel .boolean
  {
    padding-top: 0px;
    margin-top: -12px;
    /* margin-left: 0px; */
  ...
  .workflows-command-panel .string-input, .workflows-command-panel .boolean-input
  {
      height: 28px;
    }
  ...
  display: flex;
   ...
   .statuses-search-and-add .string, .statuses-search-and-add .string-input
   {
     padding: 0px
   }
