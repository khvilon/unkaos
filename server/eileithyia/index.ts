...
let execModifSqlFile = async function(file: string, name: string, workspace: string, email: string, pass: string){
    ...
    let sqlFileContentStr = sqlFileContent.replaceAll(name + '.', workspace + '.').replaceAll(name + ' ', workspace + ' ').replaceAll(name + ';', workspace + ';').replaceAll('root@unkaos.org', email).replaceAll('rootpass', pass);
