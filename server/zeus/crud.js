...
crud.jsonSql = function (table_name) {
...
crud.make_read_query_template = function (table_name, num) {
    ...
    for (let i in data_model_1.default.model[table_name].columns) {
        let column = data_model_1.default.model[table_name].columns[i];
        if (group != "")
            group += ", ";
    ...
    for (let i in data_model_1.default.model[table_name]["fk"]) {
        let fk_table_name = data_model_1.default.model[table_name]["fk"][i];
    ...
    for (let i in data_model_1.default.model[table_name]["fks"]) {
        let fk_table_name = data_model_1.default.model[table_name]["fks"][i];
...
crud.load = function () {
    return __awaiter(this, void 0, void 0, function* () {
        yield data_model_1.default.load();
        crud.querys = {};
        for (let table_name in data_model_1.default.model) {
...
crud.make_query = {
    read: function (table_name, params) {
        ...
        if ((table_name == "issues" &&
            ...
            let user_query = ' TRUE ';
            if (params["query"])
                user_query = decodeURIComponent(atob(params["query"]));
      ...
      `];
            ...
            user_query = user_query.replaceAll("I.tags=", "I.tags~");
    ...
    create: function (table_name, params) {
        ...
        let keys = "", values = "";
        let columns = data_model_1.default.model[table_name].columns;
        ...
        for (let i in params) {
            if (columns[i] === undefined ||
                ...
                i === "token_created_at")
                continue;
            if (data_model_1.default.model[table_name].columns[i] == undefined)
                continue;
            if (keys != "") {
                keys += ", ";
                values += ", ";
    ...
    update: function (table_name, params) {
        if (table_name == undefined)
            return ["", []];
        if (table_name == 'field_values' && params.field_uuid == author_field_uuid)
            return ["", []];
    ...
    upsert: function (table_name, params) {
        if (table_name == undefined)
            return ["", []];
        let [query_create, params_create] = crud.make_query.create(table_name, params);
        ...
        if (query_update == '')
            query_update = 'NOTHING';
        let [query, pg_params] = crud.concat_querys(query_create, params_create, query_update, params_update, " ON CONFLICT(uuid) DO ");
        //query += ';'
        if (table_name == "issues" && params.num == undefined) {
            let [q, p] = crud.push_query(query, pg_params, `UPDATE issues SET num = 
        ...
        WHERE uuid = $3`, [params.project_uuid, params.uuid, params.uuid]);
            query = q;
            pg_params = p;
            let [q1, p1] = crud.push_query(query, pg_params, `UPDATE issues SET status_uuid = (
                    SELECT IST.uuid 
                    FROM issue_types IT
                    ...
                    WHERE IT.uuid = $1
                    LIMIT 1
                    ) WHERE uuid = $2`, [params.type_uuid, params.uuid]);
            query = q1;
            pg_params = p1;
        }
        let subquerys = "";
        ...
        for (let i in params) {
            ...
            for (let j in params[i]) {
                let child = params[i][j];
                let [new_subqyery, new_subparams] = crud.make_query.upsert(child.table_name, child);
                //   console.log('_________0', new_subqyery)
                //   console.log('_________00')
                let [q, p] = crud.push_query(subquerys, subparams, new_subqyery, new_subparams);
                subquerys = q;
                subparams = p;
            }
        }
...
crud.get_uuids = function (obj) {
    ...
    for (let i in obj) {
        if (obj[i] === undefined ||
            ...
            continue;
        let fk = data_model_1.default.model[obj.table_name]["fk"];
        if (fk !== undefined) {
            // console.log('fk', obj.table_name, fk)
            for (let k in fk) {
                for (let j in obj[fk[k]]) {
                    if (obj[fk[k]][j].uuid == undefined)
                        ans[obj[fk[k]][j]] = fk[k];
                    else
                        ans[obj[fk[k]][j].uuid] = fk[k];
                }
            }
        }
        let fks = data_model_1.default.model[obj.table_name]["fks"];
        // console.log(obj.table_name, 'fff', fks, 'fdfdfd', obj[i][0].table_name, obj[i][0])
        if (fks == undefined || !fks.includes(obj[i][0].table_name))
            continue;
        // console.log('fff2', fks, 'fdfdfd', obj[i][0].table_name)
        for (let j in obj[i]) {
            ans = tools_1.default.obj_join(ans, crud.get_uuids(obj[i][j]));
        }
    }
    //  console.log('aaaaaaaaaans uuids', ans)
    return ans;
};
crud.do = function (subdomain, method, table_name, params, author_uuid) {
    return __awaiter(this, void 0, void 0, function* () {
        if (table_name == "issue")
            table_name = "issues";
        else if (table_name == "board")
            table_name = "boards";
        ...
        if (method != "read") {
            ...
            let data = yield sql_1.default.query(subdomain, read_query, read_params);
            ...
            if (data.rows.length > 0) {
                if (table_name == "issues" &&
                    ...
                    let action_options = {
                        ...
                        uuid: tools_1.default.uuidv4(),
                    };
                    let [ia_query, ia_params] = crud.make_query.create("issue_actions", action_options);
                ...
                for (let i in old_uuids) {
                    if (new_uuids[i] != undefined)
                        continue;
                    //   console.log('del', old_uuids[i], JSON.stringify(i))
                    if (data_model_1.default.has_fk(table_name, old_uuids[i]))
                        del_query +=
                            "delete from " +
                                table_name +
                                "_to_" +
                ...
                for (let i in new_uuids) {
                    if (old_uuids[i] != undefined)
                        continue;
                    if (!data_model_1.default.has_fk(table_name, new_uuids[i]))
                        continue;
                    del_query +=
                        "insert into " +
                            table_name +
        ...
        //  console.log('###################################################################################')
        let ans = yield sql_1.default.query(subdomain, query, pg_params);
