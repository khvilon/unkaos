...
async function init()
{
    ...
    app.post('/upsert_watcher', async (req:any, res:any) => {   
        ...
        let ans = await sql.query(subdomain, `INSERT INTO watchers SET (user_uuid, issue_uuid) VALUES('` + user.uuid + `','` + issue_uuid + `') ON CONFLICT DO NOTHING`)
    ...
    app.delete('/delete_watcher', async (req:any, res:any) => {   
        ...
        let ans = await sql.query(subdomain, `DELETE FROM watchers WHERE user_uuid = '` + user.uuid + `' AND issue_uuid = '` + issue_uuid + `'`)
    ...
    app.get('/read_watcher', async (req:any, res:any) => {   
        ...
        let ans = await sql.query(subdomain, `SELECT * FROM watchers WHERE user_uuid = '` + user.uuid + `' AND issue_uuid = '` + issue_uuid + `'`)
    ...
    for(let table in crud.querys)
    {
        for(let method in crud.querys[table])
        {
            ...
            app[dict[method]]('/' + func_name, async (req:any, res:any) => {
